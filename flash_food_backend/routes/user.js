...
router.get('/staffs', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const [staffs] = await db.query('SELECT id, email, username, full_name, birthday, phone, avatar FROM users WHERE role = ?', ['staff']);
    res.json({ staffs });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.put('/staffs/:id', authenticateToken, requireAdmin, async (req, res) => {
  ...
  const { username, full_name, birthday, phone } = req.body;
  try {
    await db.query(
      'UPDATE users SET username = ?, full_name = ?, birthday = ?, phone = ? WHERE id = ? AND role = ?',[username, full_name, birthday, phone, staffId, 'staff']
    );
...
router.get('/customers', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const [customers] = await db.query('SELECT id, email, username, full_name, birthday, phone, avatar FROM users WHERE role = ?', ['user']);
    res.json({ customers });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
